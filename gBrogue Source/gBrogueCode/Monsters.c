...
void monsterName(char *buf, creature *monst, boolean includeArticle) {
	...
	if (canSeeMonster(monst) || rogue.playbackOmniscience) {
		...
		sprintf(buf, "%s%s", (includeArticle ? (monst->creatureState == MONSTER_ALLY && !(monst->info.flags & MONST_DISTANT_FOLLOWER) ? "your " : "the ") : ""),
...
void empowerMonster(creature *monst) {
    ...
    if (canSeeMonster(monst)) {
        ...
        sprintf(buf, "you sense that %s has grown stronger", theMonsterName);
...
void updateMonsterState(creature *monst) {
	...
	enum creatureStates oldState = monst->creatureState;
...
/*
        if (monst->creatureState == MONSTER_FLEEING
            ...
            if ((monst->creatureState == MONSTER_FLEEING
                || monst->creatureState == MONSTER_TRACKING_SCENT)
                // In deep water, the player is hidden from all but whatever's in the water. Not a perfect implementation but good enough -- gsr
                && (!cellHasTerrainFlag(x, y, T_IS_DEEP_WATER) || monst->bookkeepingFlags & MB_SUBMERGED)) {
   ...
   // Give warnings on when monsters notice you.. if you can see them. -- gsr
    if (canSeeMonster(monst) && !(monst->info.flags & MB_BOUND_TO_LEADER) && !player.status[STATUS_HALLUCINATING] && !(monst->info.flags & MONST_SUBMERGES))
    {
        monsterName(monstName, monst, true);
        if (oldState == MONSTER_TRACKING_SCENT && monst->creatureState == MONSTER_WANDERING && !monst->lastSeenPlayerAt[0] == -1) // Don't want a bunch of "begins to wander" messages when we first enter a floor, so we want to check whether the monster has actually seen the player
        {
        ...
        {
            sprintf(buf, "%s gets angry", monstName);
        ...
        {
            sprintf(buf, "%s wakes up", monstName);
            message(buf, false);
        }
        else if (oldState != MONSTER_FLEEING && monst->creatureState == MONSTER_FLEEING && !(monst->info.flags & MONST_MAINTAINS_DISTANCE))
        {
            sprintf(buf, "%s begins to flee", monstName);
            combatMessage(buf, messageColorFromVictim(monst));
        }
        else if (oldState == MONSTER_FLEEING && monst->creatureState == MONSTER_TRACKING_SCENT && !(monst->info.flags & MONST_MAINTAINS_DISTANCE))
        {
            sprintf(buf, "%s is no longer fleeing", monstName);
            combatMessage(buf, messageColorFromVictim(&player));
        }
    }
}
        ...
        case BE_NEGATION:
            if (monstersAreEnemies(caster, target)) {
                ...
                if (target == &player && rogue.armor && (rogue.armor->flags & ITEM_RUNIC) && (rogue.armor->flags & ITEM_RUNIC_IDENTIFIED)
                    && rogue.armor->enchant2 == A_REFLECTION && netEnchant(rogue.armor) > 0 && rogue.armor->enchant1>0) {
...
void moveAlly(creature *monst) {
	...
	} else if ((monst->info.flags & MONST_DISTANT_FOLLOWER) || (monst->bookkeepingFlags & MB_DOES_NOT_TRACK_LEADER)
			   || (distanceBetween(x, y, player.xLoc, player.yLoc) < 3 && (pmap[x][y].flags & IN_FIELD_OF_VIEW))) {

		monst->bookkeepingFlags &= ~MB_GIVEN_UP_ON_SCENT;
        monsterMillAbout(monst, 30);
//        makeIdle(monst); // Wander rather than simply walk in place like a dummy -- gsr
...
void monstersTurn(creature *monst) {
	...
	// Peacefuls may decide to fully ally with you
        /*
        if (monst->creatureState == MONSTER_ALLY && (monst->info.flags & MONST_DISTANT_FOLLOWER)
            && canSeeMonster(monst) && distanceBetween(x, y, player.xLoc, player.yLoc) < 10 && rand_percent(3))
        {
            monsterName(buf, monst, true);
            sprintf(buf, "%s has decided to join you.", buf);
            messageWithColor(buf, &advancementMessageColor, false);
            monst->info.flags &= ~(MONST_DISTANT_FOLLOWER | MONST_WILL_NOT_USE_STAIRS);
        }*/
    ...
    if ((theItem->category & (STAFF | WAND))
        ...
        switch (boltEffectForItem(theItem)) {
            ...
            case BE_DOMINATION:
                if (monst->creatureState != MONSTER_ALLY) {
                    if (monst->info.flags & MONST_INANIMATE) {
                        sprintf(newText, "\n     A staff of domination will have no effect on objects like %s.",
                                monstName);
                    } else if (monst->info.flags & MONST_INVULNERABLE) {
                            sprintf(newText, "\n     A staff of domination will have no effect on %s.",
                                    monstName);
                    } else if (wandDominate(monst) <= 0) {
                        sprintf(newText, "\n     A staff of domination will fail at %s's current health level.",
                                monstName);
                    } else if (wandDominate(monst) >= 100) {
                        sprintf(newText, "\n     A staff of domination will always succeed at %s's current health level.",
                                monstName);
                    } else {
                        sprintf(newText, "\n     A staff of domination will have a %i%% chance of success at %s's current health level.",
                                wandDominate(monst),
                                monstName);
                    }
                    successfulDescription = true;
